...
srv_test_node::srv_test_node(const std::string node_name):Node(node_name){
    ...
    static int count = 0;
    _client_ptr = this->create_client<frhal_msgs::srv::ROSCmdInterface>("FR_ROS_API_service");
    auto locktimer_callback = [&]()->void{
        static int counter = 0;
        auto req = std::make_unique<frhal_msgs::srv::ROSCmdInterface::Request>();
        if(counter == 0){
...
void srv_test_node::_get_response_callback(rclcpp::Client<frhal_msgs::srv::ROSCmdInterface>::SharedFutureWithRequest future){
